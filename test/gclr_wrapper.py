...
def test_widowx_gclr_q1():
    env_widowx = create_widow_env()

    env_widowx = GoalConditionedEnv(env_widowx, **env_config)
    print("---------------------------------------")
    # print(env_widowx.unwrapped.env.original_object_positions)
    # env_widowx.unwrapped.set_target_position(np.array([0,0,0]))
    
    print("env_widowx.unwrapped.robot_id",env_widowx.unwrapped.robot_id)
    for _ in range(1):
        obs, reward, done, info = env_widowx.step(env_widowx.action_space.sample())
        print("---------------------------------------")
        print(obs)
        print(env_widowx.unwrapped.env.get_observation())
        print("obs", obs.shape)
        # print(reward)
    print(env_widowx.unwrapped.env.original_object_positions)
    print("---------------------------------------")
    print(env_widowx.unwrapped.get_object_position())
    print(env_widowx.unwrapped.objects)
